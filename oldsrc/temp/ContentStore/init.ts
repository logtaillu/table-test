...
export function updateColumn(store: ContentStore, cols: ITableProps["columns"]) {
    const { insertCols = [], hiddenCols = [], colCount = 0 } = store.content;
}

export function updateData(store: ContentStore, data: ITableProps["data"], rowkey: ITableProps["rowkey"]) {
    const { insertRows = [], hiddenRows = [], rowCount = 0 } = store.content;
